...
class PropertiesDialog() {
    ...
    constructor(activity: Activity, path: String, countHiddenItems: Boolean = false) : this() {
        ...
        if (fileDirItem.isDirectory) {
            addProperty(R.string.last_modified,convertIntoDate(fileDirItem.getLastModified(activity)))
        } else {
    ...
    private fun updateLastModified(activity: Activity, view: View, timestamp: Long) {
        activity.runOnUiThread {
            view.findViewById<TextView>(R.id.properties_last_modified).property_value.text = convertIntoDate(timestamp)
...
//            property_label.typeface = Typeface.DEFAULT_BOLD
