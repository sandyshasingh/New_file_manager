...
class AudioFetcher(var context: Context,var fetchAudioAsyncCompleteListener: FetchAudioAsyncCompleteListener) : AsyncTask<Void, Void, List<ListItem>>() {
   ...
   // var audios: MutableLiveData<List<ListItem>>? = MutableLiveData()
    ...
    private fun getAudios(context: Context): List<ListItem>? {
...
//                            item.sizeCalculator = totalSize
    ...
    override fun onPostExecute(result: List<ListItem>?) {
        ...
        fetchAudioAsyncCompleteListener.fetchAudioCompleted(result)
    }
    interface FetchAudioAsyncCompleteListener {
        fun fetchAudioCompleted(audiosList:List<ListItem>?)
    }
