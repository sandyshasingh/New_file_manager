...
import com.editor.hiderx.HiderUtils.ORIENT_LANDSCAPE_LEFT
import com.editor.hiderx.HiderUtils.ORIENT_LANDSCAPE_RIGHT
import com.editor.hiderx.Utility.FLASH_AUTO
import com.editor.hiderx.Utility.FLASH_ON
import com.editor.hiderx.Utility.STATE_PICTURE_TAKEN
import com.editor.hiderx.Utility.STATE_PREVIEW
import com.editor.hiderx.Utility.STATE_RECORDING
import com.editor.hiderx.Utility.STATE_STARTING_RECORDING
import com.editor.hiderx.Utility.STATE_STOPING_RECORDING
import com.editor.hiderx.Utility.STATE_WAITING_LOCK
import com.editor.hiderx.Utility.STATE_WAITING_NON_PRECAPTURE
import com.editor.hiderx.Utility.STATE_WAITING_PRECAPTURE
...
class CameraPreview : ViewGroup, TextureView.SurfaceTextureListener, MyPreview  {
    ...
    private var mLastVideoName: String = ""
    ...
    private var mCameraState = Utility.STATE_INIT
    private var mFlashlightState = Utility.FLASH_OFF
    ...
    constructor(activity: CameraActivity, textureView: AutoFitTextureView, initPhotoMode: Boolean) : super(
        activity
    ) {
        mActivity = activity
        ...
        val cameraCharacteristics = try {
            getCameraCharacteristics(
                HiderUtils.getIntSharedPreference(
                    activity,
                    HiderUtils.LAST_USED_CAMERA_ID
                ).toString()
            )
        } catch (e: Exception) {
            if(BuildConfig.DEBUG)
            Toast.makeText(activity, "Get camera characteristics $e", Toast.LENGTH_LONG).show()
            FirebaseCrashlytics.getInstance().log(e.toString())
            FirebaseCrashlytics.getInstance().recordException(e)
            null
        }

        val isFrontCamera = cameraCharacteristics?.get(CameraCharacteristics.LENS_FACING).toString() == MyCameraImpl(
            activity
        ).getFrontCameraId().toString()
        mUseFrontCamera = HiderUtils.getBooleanSharedPreference(
            activity,
            HiderUtils.ALWAYS_OPEN_BACK_CAMERA
        ) && isFrontCamera
    ...
    private fun openCamera(width: Int, height: Int) {
        try {
            ...
            Toast.makeText(mActivity, "Open camera $e", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private val imageAvailableListener = ImageReader.OnImageAvailableListener { reader ->
        try {
            ...
            try {
                newFileName = "Image"+Math.random()+".jpg"
                newFile = File(
                    StorageUtils.getPhotosHiderDirectory() + "/" + StorageUtils.encode(
                        newFileName,
                        StorageUtils.offset
                    )
                )
                out = FileOutputStream(newFile)
                    out.write(bytes)
                out.flush()
                mActivity.insertPhotoInDb(
                    HiddenFiles(
                            newFile.path,
                            newFileName,
                            StorageUtils.getPublicAlbumStorageDirForPhotos()?.path + "/" + newFileName,
                            StorageUtils.format(
                                newFile.length().toDouble(),
                                2
                            ),
                            "image/*",
    ...
    private fun getCurrentResolution(): MySize {
        val configMap = mCameraCharacteristics?.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP) ?: return MySize(
            0,
            0
        )
        val resIndex = if (mUseFrontCamera) {
            if (mIsInVideoMode) {
                HiderUtils.getIntSharedPreference(mActivity, HiderUtils.FRONT_VIDEO_RES_INDEX_KEY)
            } else {
                HiderUtils.getIntSharedPreference(mActivity, HiderUtils.FRONT_PHOTO_RES_INDEX_KEY)
            }
        } else {
            if (mIsInVideoMode) {
                HiderUtils.getIntSharedPreference(mActivity, HiderUtils.BACK_VIDEO_RES_INDEX_KEY)
            } else {
                HiderUtils.getIntSharedPreference(mActivity, HiderUtils.BACK_PHOTO_RES_INDEX_KEY)
            }
        }
    ...
    private fun setupCameraOutputs(width: Int, height: Int) {
        ...
        try {
            ...
            Toast.makeText(mActivity, "Setup camera outputs $e", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun chooseOptimalPreviewSize(
        ...
        return when {
            bigEnough.isNotEmpty() -> bigEnough.minByOrNull { it.width * it.height }?:selectedResolution.toSize()
            notBigEnough.isNotEmpty() -> notBigEnough.maxByOrNull { it.width * it.height }?:selectedResolution.toSize()
            bigEnoughIncorrectAR.isNotEmpty() -> bigEnoughIncorrectAR.minByOrNull { it.width * it.height }?:selectedResolution.toSize()
            notBigEnoughIncorrectAR.isNotEmpty() -> notBigEnoughIncorrectAR.maxByOrNull { it.width * it.height }?:selectedResolution.toSize()
    ...
    private fun getFrameRange(): Range<Int> {
        val ranges = mCameraCharacteristics?.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES) ?: return Range<Int>(
            0,
            1
        )
        var currRangeSize = -1
    ...
    private fun captureStillPicture() {
        try {
            ...
            /*if (HiderUtils.getBooleanSharedPreference(mActivity,HiderUtils.IS_SOUND_ENABLED)) {*/
           ...
           // }
            ...
            Toast.makeText(mActivity, "Capture picture $e", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun getCameraCharacteristics(cameraId: String = mCameraId) = getCameraManager().getCameraCharacteristics(
        cameraId
    )
    ...
    private fun setupMediaRecorder() {
        try {
            ...
            mLastVideoName = "VID"+Math.random().toString()+".mp4"
            mLastVideoPath = StorageUtils.getVideosHiderDirectory() +"/"+StorageUtils.encode(
                mLastVideoName,
                StorageUtils.offset
            )
            val uri = Uri.fromFile(File(mLastVideoPath))
    ...
    private fun startRecording() {
        ...
        closeCaptureSession()
        setupMediaRecorder()
       // if (HiderUtils.getBooleanSharedPreference(mActivity, IS_SOUND_ENABLED)) {
    ...
    private fun stopRecording() {
        ...
        try {
            ...
            mActivity.insertVideoInDb(
                HiddenFiles(
                        mLastVideoPath,
                        mLastVideoName,
                        StorageUtils.getPublicAlbumStorageDirForVideos()?.path!! + "/" + mLastVideoName,
                        StorageUtils.format(
                            File(
                                mLastVideoPath
                            ).length().toDouble(), 2
            ...
            Toast.makeText(mActivity, R.string.video_recording_failed, Toast.LENGTH_LONG).show()
            openResolutionsDialog(true)
            File(mLastVideoPath).delete()
        } finally {
            Thread {
                closeCamera()
                openCamera(mTextureView.width, mTextureView.height)
    ...
    private fun getAvailableVideoSizes(configMap: StreamConfigurationMap) = configMap.getOutputSizes(
        MediaRecorder::class.java
    ).filter {
        it.width <= MAX_VIDEO_WIDTH && it.height <= MAX_VIDEO_HEIGHT
    }

    private fun shouldLockFocus() = mIsFocusSupported && HiderUtils.getBooleanSharedPreference(
        mActivity,
        HiderUtils.FOCUS_BEFORE_CAPTURE
    )
    ...
    private fun openResolutionsDialog(openVideoResolutions: Boolean) {
        ...
        val photoResolutions = configMap.getOutputSizes(ImageFormat.JPEG).map { MySize(
            it.width,
            it.height
        ) } as ArrayList
